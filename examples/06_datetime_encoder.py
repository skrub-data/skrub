...
X_train_zoomed, X_test_zoomed = X_zoomed[X_zoomed["date.utc"] < "2019-06-03"], X_zoomed[
    X_zoomed["date.utc"] >= "2019-06-03"]
...
# Feature importances
# ----------------------------
# Using the DatetimeEncoder allows us to better understand how the date impacts the NO2 concentration.
# To this aim, we can compute the importance of the features created by the Datetime encoder, using
# the `permutation_importance` function, which basically shuffles a feature and sees how the model
...
sup_vec = SuperVectorizer(datetime_transformer=DatetimeEncoder(add_day_of_the_week=True))
X_ = sup_vec.fit_transform(X)  # in this case, we don't use a pipeline, because we want to compute the importance
# of the features created by the DatetimeEncoder
