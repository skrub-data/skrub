...
def get_nan_datetime(px, as_array=False):
    df = px.DataFrame(
        [
            ...
            ["2022-01-01 23:23:43", "2020-12-25 11:12:00", NULL[px.__name__]],
        ],
    )
    if as_array:
        return df.to_numpy()
...
def test_fit(
    ...
    X = get_data_func(px, as_array=as_array)
    enc = DatetimeEncoder(
