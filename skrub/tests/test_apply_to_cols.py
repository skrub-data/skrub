...
class Mult(BaseEstimator):
    """Dummy to test the different kinds of output supported by ApplyOnEachCol.
    ...
    single-column transformer, X is passed by ApplyOnEachCol as a dataframe
...
class Rejector(SingleColumnTransformer):
    """Dummy class to test ApplyOnEachCol behavior when columns are rejected."""
...
def test_allowed_column_rejections(df_module, use_fit_transform):
    ...
    mapper = ApplyOnEachCol(Rejector(), allow_reject=True)
    if use_fit_transform:
...
def test_forbidden_column_rejections(df_module):
    df = df_module.example_dataframe
    mapper = ApplyOnEachCol(Rejector())
...
def test_rejection_forbidden_in_transform(df_module):
    df = df_module.example_dataframe
    mapper = ApplyOnEachCol(RejectInTransform(), allow_reject=True)
    mapper.fit(df)
    with pytest.raises(ValueError, match=".*failed on.*int-col"):
        mapper.transform(df)
...
def test_column_renaming(df_module, use_fit_transform):
    ...
    mapper = ApplyOnEachCol(RenameB())
    fit_transform()
    assert out_names == ["B__skrub_XXX__", "B"]

    mapper = ApplyOnEachCol(RenameB(), cols=("A",), rename_columns="{}_out")
    fit_transform()
    assert out_names == ["B_out", "B"]

    mapper = ApplyOnEachCol(
...
def test_wrong_transformer_output_type(pd_module):
    with pytest.raises(TypeError, match=".*fit_transform returned a result of type"):
        ApplyOnEachCol(NumpyOutput()).fit_transform(pd_module.example_dataframe)
...
def test_set_output_failure(df_module):
    ...
    y = [0.0, 1.0]
    mapper = ApplyOnEachCol(make_pipeline(Mult()))
...
def test_output_index(cols):
    ...
    transformer = ApplyOnEachCol(ResetsIndex(), cols=cols)
...
def test_set_output_polars(pl_module):
    ...
    ApplyOnEachCol(OneHotEncoder(sparse_output=False)).fit_transform(df)
