...
from typing import Any, Optional as TypingOptional, Union, TypeVar
...
T = TypeVar('T')
DistributionType = Union[str, stats.rv_continuous, stats.rv_discrete]
...
class BaseChoice:
    ...
    def to_dict(self):
        """Serialize choice to dictionary."""
        result = {'type': self.__class__.__name__}
        for field in dataclasses.fields(self):
            if field.name not in ['chosen_outcome', '_distrib', '_original_distrib', 'constraints']:
                value = getattr(self, field.name)
                # Handle special cases
                if isinstance(value, np.ndarray):
                    value = value.tolist()
                elif isinstance(value, stats.rv_continuous) or isinstance(value, stats.rv_discrete):
                    # Skip distribution objects in serialization
                    continue
                result[field.name] = value
        # Handle constraints specially
        if hasattr(self, 'constraints') and self.constraints:
            # Can't serialize callables, just note their presence
            result['constraints'] = f"<{len(self.constraints)} constraints>"
        return result

    @classmethod
    def from_dict(cls, data):
        """Deserialize choice from dictionary."""
        # Remove type field and handle constraints
        data = {k: v for k, v in data.items() if k != 'type'}
        if 'constraints' in data:
            data.pop('constraints')  # Constraints can't be deserialized
        return cls(**data)
...
def _check_match_keys(outcomes, match_keys, match_has_default):
    try:
        ...
        examples = [repr(outcome) for outcome in list(outcomes)[:3]]
        if len(outcomes) > 3:
            examples.append("...")
        raise TypeError(
            f"To use `match()`, all choice outcomes must be hashable. {e}\n"
            f"Problematic outcomes (first 3): {examples}"
        )
    if extra_keys:
        raise ValueError(
...
def _check_constraints(value, constraints):
...
@dataclasses.dataclass
class DependentChoice(BaseChoice):
    """A choice that depends on another choice's value."""
    ...
    def __post_init__(self):
        _check_name(self.name)
        if not callable(self.mapping_func):
            raise TypeError("mapping_func must be callable")
    
    def default(self):
        """Get the default outcome based on base choice's default."""
        return self.mapping_func(get_default(self.base_choice))
    
    def chosen_outcome_or_default(self):
        """Chosen outcome when it has been set, otherwise the default."""
        return self.mapping_func(get_chosen_or_default(self.base_choice))
    
    def rvs(self, size=None, random_state=None):
        """Sample random values based on base choice's distribution."""
        base_samples = self.base_choice.rvs(size, random_state)
        if size is None:
            return self.mapping_func(base_samples)
        else:
            return [self.mapping_func(sample) for sample in base_samples]
    
    def __repr__(self):
        args_r = _utils.repr_args(
            (self.base_choice, self.mapping_func),
            {"name": self.name},
            {"name": None}
        )
        return f"dependent_choice({args_r})"


@dataclasses.dataclass
class CategoricalChoice(Choice):
    """Choice with non-uniform probabilities."""
    ...
    def __post_init__(self):
        ...
        if self.probabilities is None:
            self.probabilities = [1/len(self.outcomes)] * len(self.outcomes)
        elif len(self.probabilities) != len(self.outcomes):
            raise ValueError("lengths of `probabilities` and `outcomes` do not match")
        if not np.allclose(sum(self.probabilities), 1.0):
            raise ValueError("probabilities must sum to 1")
    
    def rvs(self, size=None, random_state=None):
        """Sample random values with given probabilities."""
        random_state = check_random_state(random_state)
        idx = random_state.choice(len(self.outcomes), size=size, p=self.probabilities)
        if size is None:
    ...
    def __repr__(self):
        ...
        args_r = _utils.repr_args(
            (arg,),
            {"probabilities": self.probabilities, "name": self.name},
            {"probabilities": None, "name": None}
        )
        return f"categorical_choice({args_r})"
...
def categorical_choice(outcomes, *, probabilities=None, name=None):
    """A choice among several outcomes with non-uniform probabilities.

    Parameters
    ----------
    outcomes : list or dict
        The possible outcomes to choose from. If a dict, the values are the
        outcomes and the keys give them human-readable names.

    probabilities : list[float], optional
        Probability for each outcome. Must sum to 1. If None, uniform
        distribution is assumed.

    name : str, optional
        Name of the choice for display purposes.

    Returns
    -------
    CategoricalChoice
        A categorical choice object.
    """
    if isinstance(outcomes, typing.Mapping):
        outcome_names, outcomes = list(outcomes.keys()), list(outcomes.values())
    else:
        outcome_names = None
    return CategoricalChoice(
        outcomes, 
...
def dependent_choice(base_choice, mapping_func, *, name=None):
    """Create a choice that depends on another choice's value.

    Parameters
    ----------
    base_choice : BaseChoice
        The choice that this choice depends on.

    mapping_func : callable
        Function that maps values from the base choice to values for this choice.
    ...
    -------
    DependentChoice
        A dependent choice object.

    Examples
    --------
    >>> import skrub
    >>> n_layers = skrub.choose_int(1, 5, name='n_layers')
    >>> # Create layer sizes that depend on n_layers
    >>> layer_sizes = skrub.dependent_choice(
    ...     n_layers,
    ...     lambda n: [skrub.choose_int(32, 256) for _ in range(n)],
    ...     name='layer_sizes'
    ... )
    """
    return DependentChoice(base_choice, mapping_func, name=name)
...
def _validate_constraints(constraints):
    ...
    for constraint in constraints:
        if not callable(constraint):
            raise TypeError(f"Constraint must be callable, got {type(constraint)}")
    return list(constraints)


def _sample_with_constraints(distrib, constraints, size, random_state, max_attempts=1000):
    """Sample from distribution until constraints are satisfied."""
    if not constraints:
        return distrib.rvs(size=size, random_state=random_state)
    
    if size is None:
        for _ in range(max_attempts):
            value = distrib.rvs(random_state=random_state)
            if _check_constraints(value, constraints):
                return value
        raise ValueError(f"Could not sample value satisfying constraints after {max_attempts} attempts")
    else:
        values = []
        for _ in range(max_attempts * size):  # Allow more attempts for multiple samples
            value = distrib.rvs(random_state=random_state)
            if _check_constraints(value, constraints):
                values.append(value)
                if len(values) == size:
                    return np.array(values)
        raise ValueError(f"Could not sample {size} values satisfying constraints")


def _create_distribution(low, high, log, to_int, distribution):
    """Create a distribution object based on parameters."""
    # Handle offset for integer sampling
    offset = 1 - 1e-6 if to_int else 0
    
    if distribution is None or distribution == 'uniform':
        if log:
            return stats.loguniform(low, high + offset)
        else:
            return stats.uniform(low, high - low + offset)
    elif distribution == 'normal':
        # For normal, low and high are used as mean and std
        mean = (low + high) / 2
        std = (high - low) / 4  # So that ~95% of values are within [low, high]
        return stats.norm(loc=mean, scale=std)
    elif distribution == 'exponential':
        # For exponential, low is the minimum, scale is derived from high
        scale = high - low
        # Shift distribution to start at low
        return stats.expon(loc=low, scale=scale)
    elif distribution == 'lognormal':
        if low <= 0:
            raise ValueError("For lognormal distribution, low must be > 0")
...
def _repr_numeric_choice(choice):
    args = _utils.repr_args(
        (choice.low, choice.high),
        {
            ...
            "distribution": choice.distribution if hasattr(choice, 'distribution') else None,
            "constraints": choice.constraints if hasattr(choice, 'constraints') and choice.constraints else None,
        },
        defaults={
            "log": False, 
            "n_steps": None, 
...
class NumericChoice(BaseNumericChoice):
    ...
    constraints: list[Callable[[float], bool]] | None = None
    ...
    def __post_init__(self):
        ...
        self.constraints = _validate_constraints(self.constraints)
        
        # Create distribution
        self._original_distrib = _create_distribution(
            self.low, self.high, self.log, self.to_int, self.distribution
        )
        
        # Check default value against constraints
        if self.default_outcome is not None:
            if not _check_constraints(self.default_outcome, self.constraints):
                raise ValueError(f"Default value {self.default_outcome} violates constraints")
        
        # For backward compatibility, keep self._distrib
        self._distrib = self._original_distrib

    def rvs(self, size=None, random_state=None):
        # Sample with constraints
        values = _sample_with_constraints(
            self._distrib, 
            self.constraints, 
            size, 
            random_state
    ...
    def default(self):
        ...
        for _ in range(1000):
            candidate = self._sample_default_candidate()
            if _check_constraints(candidate, self.constraints):
                return candidate
        
        # If no valid candidate found, use midpoint
    ...
    def _sample_default_candidate(self):
        """Sample a candidate for default value."""
        if hasattr(self, '_original_distrib'):
            candidate = self._original_distrib.rvs(random_state=42)  # Fixed seed for reproducibility
        else:
            # Fallback to midpoint
            low, high = self.low, self.high
            if self.log:
                low, high = np.log(low), np.log(high)
            candidate = np.mean([low, high])
            if self.log:
                candidate = np.exp(candidate)
        
        if self.to_int:
            candidate = np.round(candidate).astype(int)
...
class DiscretizedNumericChoice(BaseNumericChoice, Sequence):
    ...
    def __post_init__(self):
        ...
        # Apply constraints
        if self.constraints:
            self.grid = np.array([x for x in self.grid if _check_constraints(x, self.constraints)])
            if len(self.grid) == 0:
                raise ValueError("No valid values in grid satisfy the constraints")
        
        # Check default value
        if self.default_outcome is not None:
            if not _check_constraints(self.default_outcome, self.constraints):
                raise ValueError(f"Default value {self.default_outcome} violates constraints")
            if self.default_outcome not in self.grid:
                # Find closest value in grid
                idx = np.abs(self.grid - self.default_outcome).argmin()
                self.default_outcome = self.grid[idx]
...
def choose_float(
    ...
    *, 
    ...
    distribution : str or scipy.stats distribution, optional
        The distribution to use for sampling. Can be 'uniform', 'normal',
        'exponential', 'lognormal', or a scipy.stats distribution object.
        Default is 'uniform' (or 'loguniform' if log=True).

    constraints : list of callables, optional
        List of constraint functions that take a value and return True if the
        value is valid. Useful for restricting the search space to valid values.
    ...
    >>> import skrub
    >>> # With constraints
    >>> batch_size = skrub.choose_int(
    ...     1, 100, 
    ...     name='batch_size',
    ...     constraints=[
    ...         lambda x: x % 8 == 0,  # Must be divisible by 8
    ...         lambda x: x >= 16,     # Minimum batch size
    ...     ]
    ... )
...
def choose_int(
    ...
    *, 
    ...
    ...     10, 1000,
    ...     log=True,
    ...     constraints=[lambda x: x % 10 == 0],  # Must be multiple of 10
    ...     name='n_estimators'
    ... )
    """
    if n_steps is None:
...
def validate_choice_config(choice, config):
    """Validate that a configuration is valid for the choice.
    ...
    -------
    bool
        True if the configuration is valid for the choice.
    """
    if isinstance(choice, NumericChoice):
        if not (choice.low <= config <= choice.high):
            return False
        if choice.to_int and not isinstance(config, numbers.Integral):
            return False
        if hasattr(choice, 'constraints') and choice.constraints:
            return _check_constraints(config, choice.constraints)
    elif isinstance(choice, Choice):
        return config in choice.outcomes
    return True


def get_valid_configs(choice, configs):
    ...
    -------
    list
        List of configurations that are valid for the choice.
    """
    return [c for c in configs if validate_choice_config(choice, c)]


# Function to transform choice spaces
def transform_choice(choice, transform_func, inverse_func=None):
    """Apply a transformation to a choice's space.
    ...
    transform_func : callable
        Function to transform values from original space to new space.
    inverse_func : callable, optional
        Function to transform values back from new space to original space.
        
    Returns
    -------
    DependentChoice
        A choice in the transformed space.
    """
    return DependentChoice(
        choice,
        transform_func,
        name=f"transformed({choice.name})" if choice.name else None
    )
