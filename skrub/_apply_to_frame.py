...
__all__ = ["ApplySubFrame"]

# By default, select all columns
_SELECT_ALL_COLUMNS = selectors.all()


class ApplySubFrame(TransformerMixin, BaseEstimator):
    ...
    >>> ApplySubFrame(PCA(n_components=2)).fit_transform(df).round(2)
         pca0   pca1
    0 -249.01 -33.18
    1 -249.04 -33.68
    2 -252.37  66.64
    ...
    >>> pca = ApplySubFrame(PCA(n_components=2), cols=["a", "b"], keep_original=True)
    >>> pca.fit_transform(df).round(2)
         a     b      c       d  pca0  pca1
