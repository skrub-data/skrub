...
def get_namespace(dataframes):
    ...
    if _is_module(dataframes, "pandas"):
        ...
        if agg_pl.POLARS_SETUP:
            import polars as pl

            if all([(table, pl.DataFrame) for table in dataframes]) or all(
                ...
                raise ValueError(
                    "Mixing polars lazyframes and dataframes is not supported."
                )
        else:
            raise ImportError("polars dataframes used but polars is not installed.")
...
class AggJoiner(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        """
        self.check_cols(X)
        agg_px = get_namespace([table for (table, _, _) in self.tables_])
    ...
    def check_cols(self, X):
        ...
        if len(missing_cols) > 0:
            raise ValueError(
                f"Got main_key={self.main_key!r}, but column not in"
        ...
        for idx, t_tuple in enumerate(self.tables):
            if len(t_tuple) == 2:
                ...
                table, cols_to_join = t_tuple
                cols_to_agg = list(set(table.columns) - set(cols_to_join))
                tables_.append((table, cols_to_join, cols_to_agg))
            elif len(t_tuple) == 3:
                tables_.append(t_tuple)
            else:
                raise ValueError(
        ...
        for idx, (table, cols_to_join, cols_to_agg) in enumerate(self.tables_):
            ...
            if isinstance(table, str):
                ...
                table = X.copy()
                self.tables_[idx] = (table, cols_to_join, cols_to_agg)
        ...
        if self.suffixes is None:
            if n_tables == 1:
                ...
                suffixes = [f"_{idx+1}" for idx in range(len(self.tables_))]
...
class AggTarget(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        agg_px = get_namespace([X, y])
        y_type = type_of_target(y)

        self.check_cols(X, y)
        y[self.main_keys_] = X[self.main_keys_]
    ...
    def check_cols(self, X, y):
        ...
        if len(missing_cols) > 0:
            raise ValueError(
                f"Got main_key={self.main_key!r}, but column "
                f"not in X.columns: {list(X.columns)}."
            )
        self.cols_to_agg_ = y.columns
        self.main_keys_ = main_keys
        ...
        if self.suffixes is None:
            ...
            if len(self.suffixes) != len(main_keys):
                raise ValueError(
                    "Suffixes must be None or match the "
                    f"number of tables, got: '{self.suffixes}'"
                )
