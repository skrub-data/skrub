...
def _raise_if_unregistered_type(obj, kind="object"):
...
def dataframe_module_name(obj):
    ...
    raise _raise_if_unregistered_type(obj)
...
@dispatch
def to_list(col):
    raise _raise_if_unregistered_type(col, kind="Series")
...
@dispatch
def to_numpy(col):
    raise _raise_if_unregistered_type(col, kind="object")
...
def concat(*dataframes, axis=0):
    ...
    raise _raise_if_unregistered_type(dataframes[0])
...
@dispatch
def col(df, col_name):
    raise _raise_if_unregistered_type(df, kind="DataFrame")
