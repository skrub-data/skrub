...
def aggregate(
    ...
    It uses the ``dataframe.groupby(key).agg`` method from Pandas.
    ...
    if named_agg:
        base_group = table.groupby(cols_to_join).agg(**named_agg)
    else:
        base_group = None
    ...
    for output_key, (col_to_agg, kwargs) in value_counts.items():
        ...
        # TODO: perform this atleast_1d upstream in 'check_input'.
...
def join(
    ...
    It only uses the ``dataframe.merge`` method from Pandas.
