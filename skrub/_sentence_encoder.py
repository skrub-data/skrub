...
class SentenceEncoder(SingleColumnTransformer, TransformerMixin):
    ...
    .. note::
        ...
        This class uses a pre-trained model, meaning that calling ``fit`` or
        ``fit_transform`` will not train or fine-tune the model. Instead, the model is
        loaded from disk, and PCA is fitted when ``n_components != 'all'``.

    When PCA is disabled, this class is essentially stateless, with loading the
    pre-trained model from disk being the only difference between
    ``fit_transform`` and ``transform``.

    Be aware that parallelizing this class, such as using
    ...
    for each thread. Check your selected model's characteristics if memory usage
    is a concern.

    Parameters
    ----------
    model_name_or_path : str, default="intfloat/e5-small-v2"
    ...
    n_components : int or "all", default=30,
        ...
        Set ``n_components="all"`` to skip the PCA dimension reduction mechanism.
    ...
    norm : {'l1', 'l2', 'max'} or None, default='l2'
    ...
    def fit_transform(self, X, y=None):
        ...
        self.cache_folder_ = get_data_dir(
            name=self.model_name_or_path, data_home=self.cache_folder
        )
        try:
            ...
            raise ModelNotFound(
                f"{self.model_name_or_path} is not a local folder and is not a valid "
        ...
        if self.n_components != "all":
            if (min_shape := min(X_out.shape)) >= self.n_components:
                ...
                warnings.warn(
                    ...
                    "Set n_components='all' to keep all dimensions."
                )
        ...
        if self.norm is not None:
            X_out = normalize(X_out, self.norm, axis=1, copy=False)
    ...
    def _check_params(self):
        ...
        if (
            not isinstance(self.n_components, numbers.Integral)
            and self.n_components != "all"
        ):
            raise ValueError(
        ...
        if self.norm is not None and self.norm not in (options := {"l2", "l1", "max"}):
            raise ValueError(f"Got norm={self.norm} but expected any of {options}.")
        ...
        if not isinstance(self.model_name_or_path, (str, Path)):
            raise ValueError(
                f"Got model_name_or_path={self.model_name_or_path} "
                "but expected a str or a Path type."
            )
        return
