...
| {{ col.position }} | {% if col.nulls_level != 'ok' %}**`{{ col.name }}`**{% else %}`{{ col.name }}`{% endif %} | {{ col.dtype }} | {{ col.n_unique }} ({{ "%.1f" | format(col.unique_proportion * 100) }}%) | {{ col.null_count }} ({{ "%.2f" | format(col.null_proportion * 100) }}%) | {{ col.is_high_cardinality }} | {{ col.value_is_constant }} |
{% endfor -%}
...
Only the top-10 associations are reported. Values of Cramér's V above 0.9 are in
**bold**.
{% if summary.top_associations %}
| Left column | Right column | Cramér's V |
|---|---|---|
{% for assoc in summary.top_associations[:10] %}| {{ assoc.left_column_name }} | {{ assoc.right_column_name }} | {% if assoc.cramer_v > 0.9 %}**{{ "%.4f" | format(assoc.cramer_v) }}**{% else %}{{ "%.4f" | format(assoc.cramer_v) }}{% endif %} |
{% endfor %}
{% else %}
