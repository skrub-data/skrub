...
def test_missing_X_or_y():
    ...
    with pytest.raises(
        ...
        e = X.skb.apply(LogisticRegression(), y=y.skb.mark_as_y())
        skrub.cross_validate(e, environment=env)
    with pytest.raises(
        ValueError, match=r'expr should have a node marked with "mark_as_y\(\)"'
    ):
        e = X.skb.mark_as_X().skb.apply(LogisticRegression(), y=y)
        skrub.cross_validate(e, environment=env)
    # now both are correctly marked:
    e = X.skb.mark_as_X().skb.apply(LogisticRegression(), y=y.skb.mark_as_y())
    skrub.cross_validate(e, environment=env)
