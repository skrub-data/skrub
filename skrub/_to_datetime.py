...
class ToDatetimeCol(BaseEstimator):
    def fit_transform(self, column):
        ...
        if not _is_column_datetime_parsable(column):
            raise NotImplementedError()

        self.datetime_format_ = _guess_datetime_format(
            column.astype(str), random_state=0
        )
        return self.transform(column)

    def transform(self, column):
        return pd.to_datetime(column, format=self.datetime_format_)
