...
def test_fetch_openml_dataset():
    ...
    try:  # Try-finally block used to remove the test data directory at the end
        try:
            ...
            with pytest.raises(ValueError):
                assert fetch_openml_dataset(
                    dataset_id=0, data_directory=test_data_dir
                )
                assert fetch_openml_dataset(
            ...
            returned_info = fetch_openml_dataset(
                dataset_id=test_dataset["id"], data_directory=test_data_dir
            )
        ...
        assert returned_info["description"].startswith(
            test_dataset["desc_start"]
        )
        assert returned_info["source"] == test_dataset["url"]
        assert (
            returned_info["path"] == test_data_dir / test_dataset["csv_name"]
        )
...
def test_fetch_openml_dataset_mocked(
    ...
    mock_download.assert_called_with(
        dataset_id=50, data_directory=get_test_data_dir()
    )
...
def test__download_and_write_openml_dataset(mock_fetch_openml):
    ...
    if LooseVersion(sklearn.__version__) >= LooseVersion("0.22"):
        mock_fetch_openml.assert_called_once_with(
            data_id=1, data_home=str(test_data_dir), as_frame=True
        )
    else:
        mock_fetch_openml.assert_called_once_with(
            data_id=1, data_home=str(test_data_dir)
        )
...
def test__read_json_from_gz(mock_pathlib_path_isfile):
    ...
    with mock.patch(
        "gzip.open",
        mock_open(read_data='{"data": "This is JSON-encoded data!"}'),
    ) as _:
...
def test__export_gz_data_to_csv():
    ...
    with mock.patch(
        ...
        with mock.patch(
            "gzip.open", mock_open(read_data=arff_data)
        ) as mock_gzip_open:
            _export_gz_data_to_csv(dummy_gz, dummy_csv, features)
            mock_pathlib_path_open.assert_called_with(
                mode="w", encoding="utf8"
            )
            mock_gzip_open.assert_called_with(
                dummy_gz, mode="rt", encoding="utf8"
            )
...
def test_import_all_datasets(
    mock_fetch_dataset_as_namedtuple, mock_fetch_openml_dataset
):
