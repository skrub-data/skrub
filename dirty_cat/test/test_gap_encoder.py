...
def test_analyzer(init1, analyzer1, analyzer2):
    ...
    np.testing.assert_raises(
        AssertionError, np.testing.assert_array_equal, y, y2
    )
...
def test_missing_values(missing):
    ...
    if missing == "error":
        with pytest.raises(
            ValueError, match=r"Input data contains missing values."
        ):
...
if __name__ == "__main__":
    ...
    for _ in range(3):
        ...
        print(
            "{:.4} seconds".format(
                profile_encoder(GapEncoder, init="k-means++")
            )
        )
    for _ in range(3):
        print('time profile_encoder(GapEncoder, init="k-means")')
        print(
            "{:.4} seconds".format(profile_encoder(GapEncoder, init="k-means"))
        )
