...
def _random_common_char_pairs(n_pairs=50, seed=1):
    ...
    # Shuffle strings
    list1 = [
        "".join(rng.choice(list(s), size=len(s), replace=False)) for s in list1
    ]
    list2 = [
        "".join(rng.choice(list(s), size=len(s), replace=False)) for s in list2
    ]
...
def _check_symmetry(dist_func, *args, **kwargs):
    for (a, b) in _random_string_pairs():
        assert dist_func(a, b, *args, **kwargs) == dist_func(
            b, a, *args, **kwargs
        )


def _check_levenshtein_distances():
    for levenshtein_dist in [string_distances.levenshtein_seq]:
    ...
    for (a, b) in _random_string_pairs():
        assert string_distances.levenshtein_array(
            a, b
        ) == string_distances.levenshtein_seq(a, b)
        assert string_distances.levenshtein_seq(
            a, b
        ) == string_distances.levenshtein(a, b)
...
def test_compare_implementations():
    ...
    for string1, string2 in _random_common_char_pairs(n_pairs=50):
        assert string_distances._jaro_winkler(
            string1, string2, winkler=False
        ) == Levenshtein.jaro(string1, string2)
        assert string_distances._jaro_winkler(
            string1, string2, winkler=True
