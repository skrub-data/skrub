...
def check_same_transformers(
    ...
    actual_transformers_dict = dict(
        [(name, cols) for name, trans, cols in actual_transformers]
    )
...
def type_equality(expected_type, actual_type):
    """
    Checks that the expected type is equal to the actual type, assuming object
    and str types are equivalent (considered as categorical by the
    supervectorizer).
    """
    if (
        isinstance(expected_type, object) or isinstance(expected_type, str)
    ...
    return pd.DataFrame(
        {
            ...
            "str2": pd.Series(
                ["officer", "manager", "lawyer", "chef", "teacher"],
                dtype="string",
            ),
            "cat1": pd.Series(
                ["yes", "yes", "no", "yes", "no"], dtype="category"
            ),
            "cat2": pd.Series(
    ...
    return pd.DataFrame(
        {
            ...
            "float": pd.Series(
                [5.2, 2.4, 6.2, 10.45, np.nan], dtype="Float64"
            ),
            "str1": pd.Series(
            ...
            "cat1": pd.Series(
                [np.nan, "yes", "no", "yes", "no"], dtype="object"
            ),
            "cat2": pd.Series(
                ["20K+", "40K+", "60K+", "30K+", np.nan], dtype="object"
            ),
        }
    )
...
def _get_datetimes_dataframe():
    """
    Creates a DataFrame with various date format, already converted or to be
    converted.
    """
    return pd.DataFrame(
        {
...
def _test_possibilities(
    ...
    check_same_transformers(
        expected_transformers_df, vectorizer_base.transformers
    )
    ...
    check_same_transformers(
        expected_transformers_2, vectorizer_default.transformers
    )
    ...
    check_same_transformers(
        expected_transformers_series, vectorizer_base.transformers
    )
    ...
    check_same_transformers(
        expected_transformers_plain, vectorizer_cast.transformers
    )
    # With numpy
    vectorizer_cast.fit_transform(X_str.to_numpy())
    check_same_transformers(
        expected_transformers_np_cast, vectorizer_cast.transformers
    )
...
def test_auto_cast():
    ...
    for col in X_trans.columns:
        assert type_equality(
            expected_types_clean_dataframe[col], X_trans[col].dtype
        )
    ...
    for col in X_trans.columns:
        assert type_equality(
            expected_types_dirty_dataframe[col], X_trans[col].dtype
        )


def test_with_arrays():
    """
    Check that the SuperVectorizer works if we input a list of lists or a numpy
    array.
    """
...
def test_get_feature_names_out():
    ...
    if LooseVersion(sklearn.__version__) < LooseVersion("0.23"):
        ...
        expected_feature_names_pass = (
            [  # Order matters. If it doesn't, convert to set.
                "str1_private",
                "str1_public",
                "str2_chef",
                "str2_lawyer",
        ...
        assert (
            vectorizer_w_pass.get_feature_names()
            == expected_feature_names_pass
        )
        assert (
            vectorizer_w_pass.get_feature_names_out()
            == expected_feature_names_pass
        )
    ...
    assert (
        vectorizer_w_drop.get_feature_names_out()
        == expected_feature_names_drop
    )
