...
def main():
    ...
    card_vals = [20, 30, 40, 50]
    n_comp_vals = [10, 30, 50]
    ...
    logging.debug(
        f"With cardinality possibilities: {card_vals} "
        f"and n_components possibilities: {n_comp_vals}"
    )
    for pipeline, datasets in zip(
        ...
        for info, name in datasets:
            ...
            grid = GridSearchCV(
                estimator=pipeline,
                param_grid={
                    "sv__cardinality_threshold": card_vals,
                    "sv__high_card_str_transformer__n_components": n_comp_vals,
                },
                n_jobs=30,
            )
            grid.fit(X, y)
