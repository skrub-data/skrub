...
def benchmark(
    ...
    min_iter: int,
    max_iter: int,
    gamma_shape_prior: float,
    gamma_scale_prior: float,
    rho: float,
    rescale_rho: bool,
    rescale_W: bool,
    max_iter_e_step: int,
    ...
    y = ds.y
    # only keep the first 20000 rows
    # split the data into train and test
    X_train, X_test, y_train, y_test = train_test_split(
        X[:20], y[:20], test_size=0.2, random_state=42
    )
    ...
    gap = GapEncoder(
        ...
        min_iter=min_iter,
        max_iter=max_iter,
        gamma_shape_prior=gamma_shape_prior,
        gamma_scale_prior=gamma_scale_prior,
        rho=rho,
    ...
    roc_auc_logreg_train = roc_auc_score(
        y_train, clf.predict_proba(X_train_encoded)[:, 1]
    )
    roc_auc_logreg_test = roc_auc_score(y_test, clf.predict_proba(X_test_encoded)[:, 1])
    balanced_accuracy_logreg_train = balanced_accuracy_score(
        y_train, clf.predict(X_train_encoded)
    )
    balanced_accuracy_logreg_test = balanced_accuracy_score(
        y_test, clf.predict(X_test_encoded)
    )

    clf = HistGradientBoostingClassifier()
    clf.fit(X_train_encoded, y_train)
    roc_auc_hgb_train = roc_auc_score(y_train, clf.predict_proba(X_train_encoded)[:, 1])
    roc_auc_hgb_test = roc_auc_score(y_test, clf.predict_proba(X_test_encoded)[:, 1])
    ...
    res_dic = {
        ...
        "roc_auc_logreg_train": roc_auc_logreg_train,
        "roc_auc_logreg_test": roc_auc_logreg_test,
        "roc_auc_hgb_train": roc_auc_hgb_train,
        "roc_auc_hgb_test": roc_auc_hgb_test,
        "balanced_accuracy_logreg_train": balanced_accuracy_logreg_train,
        "balanced_accuracy_logreg_test": balanced_accuracy_logreg_test,
